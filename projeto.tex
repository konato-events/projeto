...
%	\includepdf{inc/ficha_catalografica.pdf}
...
Para organizar as fases de problematização e \emph{design} do negócio, escolhemos usar os diagramas de Mapa de Empatia (\autoref{sec:mapa-empatia}), Árvore de Problemas (\autoref{sec:arvore-problemas}), \emph{Lean Canvas} (\autoref{sec:lean-canvas}), e \emph{Experiment Board} (\autoref{sec:experiment-board}). Cada um desses foca no desenvolvimento metódico de uma parte importante da solução, respectivamente: o cliente em si, os problemas que ele enfrenta, os métodos de solução e desenvolvimento do negócio, e as hipóteses que precisam ser testadas para verificar a viabilidade e eficiência do negócio.
...
\subsection{\emph{Experiment Board} - Hipóteses a serem avaliadas} \label{sec:experiment-board}
...
% Possibilidades:
% - estados do usuário: comum, participante, palestrante, organizador
% - estados de um claim: recebido, em análise, aceito, recusado
% - estados de um pedido de remoção: recebido, aceito, recusado
