...
\begin{itemize}[itemsep=-1ex]
	\item \conf{} a importância que os diversos eventos, dentro e fora da sua universidade, tem para sua formação extra-curricular;
	\item \conf{} o incentivo que alguns professores dão para a participação em atividades práticas, de campo ou de integração;
	\item \conf{} como algumas oportunidades foram perdidas por ineficiência de comunicação -- seja por terem perdido um anúncio em especial na instituição ou não terem contato com ``a pessoa certa'', que indicou determinado evento para um amigo mas não para ela, por exemplo.
\end{itemize}
...
\begin{citacao}
	...
	O segredo? Se nós temos uma estratégia cardinal que forma a pedra de fundação de todas as nossas \emph{[da Semler]} práticas, pode ser isso:\\
	Perguntar por que.\\
