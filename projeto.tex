...
\usepackage{graphicx}
\usepackage[section]{placeins} % Allows for the new command \FloatBarrier, that forces the queue of floats to be cleared. The [section] changes the use of \section to always run \FloatBarrier before.
...
\data{PXI~-~2015.2~-~013}
\autor{\textbf{Igor~Santos}\\\texttt{conf@igorsantos.com.br}\\}
\orientador{Prof. Airton Sartore}
\coorientador[Coorientadora:]{Prof\textordfeminine{}. Gellars Tavares}
...
%TODO Aqui precisamos falar sobre o histórico e atividade da empresa (aka quem é o cara por trás da empresa, já que ela ainda não existe), o mercado, a concorrência e escalabilidade do negocio. Boa parte disso aparece no Canvas mas nem tudo, então tem que ficar de olho
...
\begin{description}
	...
	\item[Conferência] Uma reunião informativa composta por palestras formais. As atividades consistem de exposições de resultados por autoridades no assunto, e os questionamentos do auditório ocorrem somente no final, normalmente por escrito. O público já está familiarizado com o tópico do evento, e tem como objetivo a expansão ou atualização dos conhecimentos que já possuem. Também envolve networking, que pode gerar bons frutos, visto que os participantes em geral tem mais conhecimento sobre o tema abordado. %todo: pesquisar na internet a programação de conferencias famosas e conferir se bate com o que ta escrito aqui
...
\begin{citacao}
	...
	{\raggedright (\citetext{five-whys} Tradução própria.)}
\end{citacao}
...
\begin{citacao}
	...
	{\raggedright (\citetext{three-whys-semler} Tradução própria.)}
\end{citacao}
...
% Hipótese final: single, clear and compeling message that states why your product is different and worth buying

\paragraph*{5 - Canais de distribuição}
%uma das ideias pra divulgar o app é bem simples:
...
%de criar um blog, de postar no Facebook, Twitter, talvez fazer contato com organizações e eventos em geral vendendo a ideia tbm, pesquisando por aí eventos que existam e entrando em contato, sei lá... não sei ainda
%
%eu acho que, inicialmente, aqui no brasil eu vou divulgar por conta própria
%com amigos e afins, fazendo as pesquisas nas universidades q conheço e q consiga contato
%e la pra fora vou colocar anúncios no Google pra poder angariar pessoas que falem inglês pra rodar pesquisas na gringa tbm
...
\begin{citacao}
	...
	{\raggedright (\citetext{lean-customer-development} Cap. 1. Grifo do autor, tradução própria.)}
\end{citacao}
...
\begin{itemize}[itemsep=-1ex]
	\item Muitas atividades ainda são manuais, como por exemplo a \emph{home page} de divulgação dos eventos -- há links para \emph{hotsites} com erro no endereço (exemplo: \emph{interevent.\underline{com..br}/evento/sbdof2015}) --, certificados com nomes distintos dos usados na inscrição, dentre outros;
...
\begin{minipage}{\linewidth} 
	\begin{framed}
		...
		\begin{citacao}
			...
			{\raggedright (\citetext{heroku-docker-php} Tradução própria.)}
		\end{citacao}
	\end{framed}
\end{minipage}
...
\centering
	...
	\label{prot:home}
\end{figure}
\footnotetext{\emph{Call to Action}: também conhecido como CTA, é um termo de marketing usado para indicar botões, links ou mesmo frases avulsas que sugestionam o usuário a tomar uma ação específica -- por exemplo, fazer uma compra, efetuar um cadastro, etc.}
...
\FloatBarrier
\subsubsection{Protótipos estáticos das telas futuras}
...
\FloatBarrier
\subsubsection{Estado final das telas do sistema}
Aqui demonstraremos como as telas do projeto foram implementadas, de acordo com a versão final do sistema, imediatamente antes da entrega deste documento. Devemos observar que, atualmente, ainda não é possível fazer o \emph{upload} de imagens para representar os eventos; no lugar, estão sendo utilizados, provisoriamente, avatares geométricos únicos.

\begin{figure}[h]
\centering
	\includegraphics[width=1\linewidth]{imagens/screenshots/search.jpg}
	\caption[Busca de eventos]{Tela de busca de eventos. Referente à \autoref{prot:search}.}
	\label{screen:search}
\end{figure}

\begin{figure}[h]
\centering
	\includegraphics[width=1\linewidth]{imagens/screenshots/home-mobile.jpg}
	\caption[Página inicial, versão móvel]{Tela inicial do sistema, simulada para exibir como ela aparece em dispositivos móveis, como celulares. À esquerda, vemos a tela pura, e à direita, o menu aberto. Referente à \autoref{prot:home}.}
	\label{screen:home}
\end{figure}

\begin{figure}[h]
\centering
	\includegraphics[width=1\linewidth]{imagens/screenshots/event-details.jpg}
	\caption[Detalhes do evento]{Página que exibe todos os detalhes do evento, com endereço, data e hora, programação, e outros. Referente à \autoref{prot:event}.}
	\label{screen:event-details}
\end{figure}

\begin{figure}[h]
\centering
	\includegraphics[width=0.8\linewidth]{imagens/screenshots/event-edit.png}
	\caption[Edição das informações básicas do evento]{Formulário de edição do evento, que cobre as principais informações. Outros dados são tratados em outras partes da tela, como a programação, palestrantes, temas e materiais.}
	\label{screen:event-edit}
\end{figure}

\begin{figure}[h]
\centering
	\includegraphics[width=1\linewidth]{imagens/screenshots/user-details.png}
	\caption[Perfil do usuário]{Página que exibe o perfil de um usuário, com suas redes sociais, foto, dados pessoais e informações de uso da plataforma. Referente à \autoref{prot:profile}.}
	\label{screen:user-details}
\end{figure}

\begin{figure}[h]
\centering
	\includegraphics[width=1\linewidth]{imagens/screenshots/user-edit.jpg}
	\caption[Edição de perfil]{Formulário de edição de perfil do usuário, que já permite editar informações básicas e a foto de exibição.}
	\label{screen:user-edit}
\end{figure}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%% CASOS DE USO DO USUÁRIO NÃO-LOGADO %%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\subsubsubsection*{\UC{} Criar Perfil \label{uc:cadastro}}
...
\subsubsubsection*{\UC{} Criar Perfil a partir de Rede Social \label{uc:cadastro-social}}
...
\subsubsubsection*{\UC{} Ver perfil de usuário \label{uc:ver-perfil}}
...
\subsubsubsection*{\UC{} Editar perfil \label{uc:editar-perfil}}
\begin{enumerate}[itemsep=-1ex]
...
\subsubsubsection*{\UC{} Vincular Rede Social \label{uc:vincular-rede}}
...
\subsubsubsection*{\UC{} Requisitar acesso ao perfil social \label{uc:requisitar-perfil-social}}
...
\subsubsubsection*{\UC{} Buscar evento \label{uc:buscar-evento}}
\begin{enumerate}[itemsep=-1ex]
...
\subsubsubsection*{\UC{} Visualizar evento \label{uc:ver-evento}}
\begin{enumerate}[itemsep=-1ex]
...
\subsubsubsection*{\UC{} Visualizar mapa / localização do evento \label{uc:ver-mapa}}
...
\subsubsubsection*{\UC{} Visualizar programação do evento \label{uc:ver-programacao}}
\emph{Este Caso de Uso é incluído no \ref{uc:ver-evento}.}
\begin{enumerate}[itemsep=-1ex,topsep=-1ex]
...
\subsubsubsection*{\UC{} Visitar venda de ingressos \label{uc:ingressos}}
\begin{enumerate}[itemsep=-1ex]
	\item O usuário requisita a programação de um evento;
	\item O Sistema exibe a grade de atividades do evento.
\end{enumerate}

\subsubsubsection*{\UC{} Ver tema \label{uc:ver-tema}}
\begin{enumerate}[itemsep=-1ex]
	\item O usuário requisita os detalhes de um tema;
	\item O Sistema exibe os dados relacionados ao tema indicado.
\end{enumerate}

\subsubsubsection*{\UC{} Fazer Logout \label{uc:logout}}
...
% manter este caso de uso sempre como o último dessa subsubseção, por ser requerido globalmente pela próxima subsubseção!
\subsubsubsection*{\UC{} Fazer Login \label{uc:login}}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%% CASOS DE USO DO USUÁRIO LOGADO %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\subsubsubsection*{\UC{} Indicar-se como palestrante em evento \label{uc:indicar-palestrante}}
...
\subsubsubsection*{\UC{} Submeter palestra \label{uc:submeter-palestra}}
...
\subsubsubsection*{\UC{} Indicar participação em evento \label{uc:indicar-participacao}}
...
\subsubsubsection*{\UC{} Indicar interesse em evento \label{uc:indicar-interesse}}
...
\subsubsubsection*{\UC{} Submeter novo evento / Editar evento existente \label{uc:submeter-evento}}
...
\subsubsubsection*{\UC{} Notificar usuários \label{uc:notificar}}
...
\subsubsubsection*{\UC{} Sugerir exclusão de evento \label{uc:sugerir-exclusao}}
...
\subsubsubsection*{\UC{} Submeter novo evento completo / Editar evento existente completo \label{uc:submeter-evento-organizador}}
...
\subsubsubsection*{\UC{} Submeter material \label{uc:submeter-material}}
...
\subsubsubsection*{\UC{} Submeter palestrante \label{uc:submeter-palestrante}}
...
\subsubsubsection*{\UC{} Submeter programação \label{uc:submeter-programacao}}
\emph{Este Caso de Uso estende o \ref{uc:submeter-evento-organizador}.}
...
\subsubsubsection*{\UC{} Marcar usuário como palestrante \label{uc:marcar-palestrante}}
...
\subsubsubsection*{\UC{} Excluir evento \label{uc:excluir}}
...
Levando isto em conta os benefícios de reaproveitamento de código no uso de um \emph{framework} como o Laravel, não re-diagramamos todas as classes que não fazem parte diretamente das Regras do Negócio. Optamos, portanto, por representar de forma simplificada as estruturas de terceiros que utilizamos diretamente. Na \autoref{diag:classes-models} estão representados os \emph{models} e suas estruturas de suporte; na \autoref{diag:classes-http} ficam os \emph{controllers} e classes associadas.

Considerações adicionais:
\begin{itemize}[itemsep=-1ex,topsep=-1ex]
	\item os modelos do ORM são similares ao Modelo de Dados da \autoref{sec:mer};
	\item algumas das propriedades indicadas como públicas, nos \emph{Models}, são acessadas a partir de métodos mágicos, que são reconhecíveis pelo padrão usado no ORM do Laravel: \texttt{get<<Nome>>Attribute()} e \texttt{set<<Nome>>Attribute()};
	\item além de interfaces, uma outra estrutura especial do PHP foi utilizada no projeto: os \emph{traits}. Eles permitem composição de comportamento, além da herança tradicional, permitindo maior aproveitamento horizontal, sem repetição de código. Eles são representados pelo estereótipo \texttt{<<trait>>}, e tem cor acinzentada.
\end{itemize}

\begin{figure}
	\centering
	\includegraphics[width=0.7\linewidth]{diagramas/classes/models.png}
	\caption{Diagrama dos \emph{Models} classes relacionadas}
	\label{diag:classes-models}
\end{figure}

\begin{sidewaysfigure}
	\centering
	\includegraphics[width=1\linewidth]{diagramas/classes/http.png}
	\caption{Diagrama dos \emph{Controllers} e classes relacionadas}
	\label{diag:classes-http}
\end{sidewaysfigure}

\subsection{Diagramas de Atividade}

\subsubsection*{Criação de novo evento - \autoref{diag:act_event_new}}
Após preencher informações iniciais do evento, sugerimos ao usuário inserir mais dados, caso possua. Ele também pode requisitar posse do evento, se fizer parte da organização.
...
%todo: descrever
\begin{figure}[p!] %%%%%%%%%%%%%%%%%%%%%%%%%%%% ESTE DIAGRAMA VAI COMO A3!! %%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\centering
	\includegraphics[width=0.9\linewidth]{diagramas/comuns/sd_event_edit.png}
	\caption{Diagrama de Sequência indicando as ações durante a edição de um evento.}
...
\begin{description}
	...
	\item[Atualização de bibliotecas \emph{open-source}] Além dos quesitos funcionais, o sistema trouxe a oportunidade de contribuir para projetos de código aberto, como o \emph{framework} Laravel e a biblioteca de mapeamento de objetos relacionais Ardent. O último, em especial, é um projeto mantido pelo próprio autor e de largo uso na comunidade Laravel, sendo utilizado como principal substituto/adicional aos recursos de banco de dados já existentes no \emph{framework}.
\end{description}

\subsection{Viabilidade e Sustentabilidade econômica}
O projeto, conforme entregue ao final do processo deste TCC, estará em pleno funcionamento e pronto para utilização da população em geral. De acordo com o que foi discutido na modelagem do negócio (\autoref{sec:modelagem-negocios}), é esperado que o sistema agrade os usuários finais e seja de utilidade instantânea, mediante certa divulgação, mesmo que baseada no boca-a-boca.

Os custos de manutenção iniciais são nulos ou bem baixos, e acreditamos que parte da tarefa de marketing será desempenhada pelos próprios usuários. A médio prazo, com o sucesso da plataforma e o aumento crescente de usuários, os custos passarão a ser mais relevantes, assunto que deve ser abordado com cuidado para não causar prejuízos aos mantenedores. Por fim, o custo de desenvolvimento também é baixo, visto que, após a entrega do sistema, ele será tomado como um projeto maduro e de código aberto pela comunidade. A expectativa é de que, assim, ele não precisará de um grande esforço de desenvolvimento para se manter em funcionamento -- apenas para adicionar novas funcionalidades, por exemplo --, e receberá contribuições ocasionais da comunidade de desenvolvedores, incluindo relatórios de problemas, correções e, eventualmente até mesmo funcionalidades menores.

Com o aumento de usuários, por outro lado, será possível por em prática um dos planos de monetização do projeto: a geração de estatísticas e percepções sobre o público interessado e dados em geral sobre o público-alvo dos eventos. Tais informações podem ser apresentadas de forma agradável e eficiente aos organizadores de eventos, que em troca irão pagar uma taxa fixa para ter acesso a tais dados. Esta estratégia vai a favor de uma corrente cada vez mais crescente de monetização de dados\cite{data-monetization-wiki}\cite{data-monetization}\cite{data-service-wiki}. Inicialmente podemos oferecer dados simples, como cruzar os tópicos marcados pelos usuários e disponibilizar pesquisas individuais para o público interessado, e a longo prazo podemos pensar em estratégias mais profundas como \emph{big data} -- análise diversificada de grandes massas de dados, que poderemos gerar a partir dos acessos e formas de utilização da nossa plataforma pelos usuários. Com a monetização em prática, podemos aumentar a dedicação ao projeto, contratar uma equipe exclusiva e dar novo gás ao andamento do mesmo.

Outras estratégias de monetização mais simples incluem a criação de pacotes pagos para organizadores, fornecendo ferramentas extras como acesso ao envio de mensagens por e-mail aos interessados no evento, envio de pesquisas pré e pós-evento, dentre outros. Todas as formas de monetização citadas precisam ainda ser avaliadas e testadas a partir das metodologias citadas (\autoref{sec:modelagem-negocios}), no Lean Canvas e Experiment Board.
...
%todo avaliar forma de manter o título do apêndice junto com a primeira página
...
\subsubsubsection*{Node/io.js + NPM}
...
\subsubsubsection*{Criar um container a partir da imagem}
...
%\begin{wrapfigure}[4]{R}[10pt]{0.4\textwidth}
%	\vspace{-40pt}
%	\includegraphics[width=0.37\textwidth]{imagens/faq/notifs.jpg}
%\end{wrapfigure}
...
\subsection[É possível integrar com minha agenda/calendário?]{Eu uso o Outlook / Google Calendar / iCal para me organizar. O que você pode fazer por mim?} \label{faq:calendarios}


\begin{wrapfigure}[4]{R}{0.24\textwidth}
	\vspace{40pt}
	\includegraphics[width=0.2\textwidth]{imagens/faq/export.jpg}
\end{wrapfigure}

\adiado{} É muito fácil fazer a integração das suas atividades no Konato com o seu calendário do dia-a-dia:
\begin{enumerate}[itemsep=-1ex]
	\item A partir do seu perfil, vá em [Editar], no canto direito;
	\item Na lateral direita da tela, mais pra baixo, tem uma lista com os Provedores de Calendários que nós damos suporte;
	\item Se você clicar no tipo de Calendário que você usa, vão aparecer instruções detalhadas sobre como configurar cada um deles.
\end{enumerate}

Com a integração configurada logo vão aparecer no seu calendário os eventos que você vai participar e as atividades que você tem interesse em assistir. Achou prático? Então mostra pros seus amigos! Aposto que eles vão gostar também :)
